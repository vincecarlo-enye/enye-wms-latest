...
function addToConfig({
  ...
  const parseResult = oxcParser.parseSync("config.ts", content);
  ...
  const collectVisitor = new oxcParser.Visitor({
  ...
  const findExportVisitor = new oxcParser.Visitor({
...
function findTargetArrayOrCreationPoint(content, objectStart, objectEnd, pathParts) {
  ...
  const parseResult = oxcParser.parseSync("temp.ts", wrappedContent);
  if (parseResult.errors.length > 0) {
    return { type: "none" };
  }
