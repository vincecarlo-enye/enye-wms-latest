...
import reserveRequests from "../../assets/js/EmpReserveList.json"; // JSON file
import { Trash2, Hash, User, Clipboard, Calendar, Box, CalendarCheck } from "lucide-react";

const EmpReserveEdit = ({ selectedReserve = {}, onClose, onSave }) => {
  ...
  const [editingBorrow, setEditingBorrow] = useState({
    ...
    borrowedDate: selectedReserve.reservedDate
      ? new Date(selectedReserve.reservedDate.split("-").reverse().join("-")).toISOString().slice(0, 10)
      : "",
    returnedOn: selectedReserve.returnDate
      ? new Date(selectedReserve.returnDate.split("-").reverse().join("-")).toISOString().slice(0, 10)
      : "",
    itemType: selectedReserve.itemType || "LOCAL MATERIALS",
    borrowItems:
      selectedReserve.borrowItems?.length > 0
        ? selectedReserve.borrowItems
        : (
            reserveRequests.find(
              (r) => r.srfNo === selectedReserve.srfNo
            )?.items || []
          ).map((item) => ({
  ...
  const filteredProducts = reserveRequests
    .flatMap((r) => r.items)
    .filter((p) =>
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white rounded shadow-lg max-h-[90vh] overflow-y-auto">
      <h2 className="text-xl font-semibold mb-6">Edit Reserve Request</h2>
