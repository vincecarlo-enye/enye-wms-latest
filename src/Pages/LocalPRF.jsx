...
import localData from "../assets/js/LocalPRF.json";
import ViewLocalPRF from "../Components/layouts/ViewLocalPRF";
import Modal from "../Components/modal/Modal";
import StatusLocalPRF from "../Components/layouts/StatusLocalPRF";
...
const LocalPRF = () => {
  ...
  const [prfList] = useState(localData);
  ...
  return prfList.filter((item) => {
    ...
    const matchesTab =
      ...
      item.status.toUpperCase() ===
        activeTab.replace(" PRF", "").toUpperCase();
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <header className="flex justify-between items-center p-6 bg-white shadow-sm border-b">
        <h1 className="text-2xl font-semibold text-gray-800">Local PRF</h1>

        <button
          onClick={() => navigate("/CreateLocalPRF")}
        ...
        {filteredData.length === 0 ? (
          ...
          filteredData.map((item) => (
            <tr key={item.prfNo} className="hover:bg-orange-50 transition">
              <td className="px-4 py-4 text-sm font-medium text-gray-800">{item.prfNo}</td>
              <td className="px-4 py-4 text-sm text-gray-600">{item.requestedBy}</td>
              <td className="px-4 py-4 text-sm text-gray-600">{item.itemType}</td>
              <td className="px-4 py-4 text-sm text-gray-600">{item.clientName}</td>
              <td className="px-4 py-4 text-sm text-gray-600">{item.project}</td>
              <td className="px-4 py-4 text-sm text-gray-600">{item.poNo}</td>
