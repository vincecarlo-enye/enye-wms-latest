...
const LocalPRF = () => {
  ...
  const fetchPRF = async () => {
    try {
      ...
      const res = await PRFService.list({
        ...
        item_type: "Local item/s", // palitan kung iba value sa DB
      });
      ...
      console.error("Fetch local PRF error:", error);
      setPrfList([]);
  ...
  const filteredData = useMemo(() => {
    return prfList.filter((item) => {
      ...
      const matchesTab =
        activeTab === "ALL" ||
        status.toUpperCase() === activeTab.replace(" PRF", "").toUpperCase();
  ...
  const handleTabClick = (item) => {
    setActiveTab((prev) => (prev === item ? "ALL" : item));
    setPage(1);
  };
