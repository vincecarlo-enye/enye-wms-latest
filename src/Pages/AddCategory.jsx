...
import { Plus, Tag, AlignLeft, X } from "lucide-react";
import { useWarehouse } from "../context/WarehouseContext";

export default function AddCategoryModal({ isOpen, onClose, onAddCategory, warehouse }) {
  const { currentWarehouse } = useWarehouse();
  const isCebu = currentWarehouse === "cebu";

  // ── Theming ────────────────────────────────────────────────────────────────
  const iconBg       = isCebu ? "bg-purple-600"          : "bg-orange-600";
  const focusBorder  = isCebu ? "focus:border-purple-600" : "focus:border-orange-600";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 bg-opacity-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl relative p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 mb-2 sm:mb-0">Add Category</h2>
            <p className="text-sm text-gray-500">Create a new product category for {warehouse}</p>
          </div>
        </div>
