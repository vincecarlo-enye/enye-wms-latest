...
const EmployeePRF = () => {
  ...
  // Backend row -> shape for table + ViewEmployeePRF
  const normalizePRF = (row) => ({
    ...
    shippingInstructions: row.shipping_ins || "",
    terms: row.terms || "",
    ...
    checkedBy: row.checked_by_pl || "",
    approvedByPL: row.approved_by_pl || "",
    releasedBy: row.released_by_pl || "",
    receivedBy: row.received_by || "",

    // status always string
    status: row.prf_status || "",
  ...
  const fetchPRF = async () => {
    try {
      ...
      console.error("Fetch employee PRF error:", error);
      setPrfList([]);
  ...
  const filteredData = useMemo(() => {
    return prfList.filter((item) => {
      ...
      const tab = activeTab.replace(" PRF", "").toUpperCase();

      const matchesTab = activeTab === "ALL" || status === tab;
