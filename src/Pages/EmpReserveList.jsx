...
import { EmpReserveListService } from "../services/EmpReserveList.service";
...
const normalizeReserve = (item) => ({
  id: item.id,
  srfNo: item.srf_no || item.srf_no_ec || item.srfnoec || "",
  requestedBy: item.requested_by || item.requestedby || "",
  intendedFor: item.intended_for || item.intendedfor || "",
  usedFor: item.used_for || item.usedfor || "",
  itemType: item.item_type || item.itemtype || "",
  reservedDate: item.date_needed || item.reserved_date || "",
  returnOn: item.return_on || "",
  status: item.status || "PENDING",
  remarks: item.remarks || "",
  warehouse: item.warehouse || "",
  items: (item.items || []).map((row) => ({
    id: row.id,
    productName: row.prod_name || row.product_name || "",
    barcode: row.barcode || "",
...
const EmpReserveList = () => {
  ...
  const fetchReserveList = async () => {
    try {
      setLoading(true);
      const rows = await EmpReserveListService.list({ search });
  ...
  const handleSaveEdit = async (updatedData, reason) => {
    try {
      ...
      const saved = await EmpReserveListService.update(updatedData.id, payload);
      const normalized = normalizeReserve(saved);
      ...
      alert(error?.response?.data?.message || "Failed to update reserve request.");
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <Modal isOpen={!!selectedReserve} onClose={() => setSelectedReserve(null)}>
        <EmpReserveView
