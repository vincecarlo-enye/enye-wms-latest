...
import { EmpBorrowListService } from "../services/empborrowlist.service";
...
const normalizeBorrow = (item) => ({
  ...
  borrowedDate: item.date_needed || item.borrowed_date || "",
...
const EmpBorrowList = () => {
  ...
  const fetchBorrowList = async () => {
    try {
      setLoading(true);
      const rows = await EmpBorrowListService.list({ search });
      setBorrowList(rows.map(normalizeBorrow));
    } catch (error) {
      console.error("Fetch borrow list error:", error);
      setBorrowList([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveEdit = async (updatedData) => {
    try {
      const payload = {
        ...
        date_needed: updatedData.borrowedDate || "",
      ...
      const saved = await EmpBorrowListService.update(updatedData.id, payload);
      const normalized = normalizeBorrow(saved);
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="min-h-screen p-4 md:p-6 bg-gray-50">
        ...
        <div className="md:hidden space-y-4">
          {loading ? (
            <div className="text-center py-6 text-gray-500">Loading borrow records...</div>
          ) : filteredData.length > 0 ? (
            filteredData.map((item) => (
