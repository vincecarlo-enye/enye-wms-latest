...
const UserProfile = () => {
  ...
  const loadProfile = async () => {
    try {
      setLoading(true);
      const res = await ProfileService.getProfile(); // returns whole response
      const data = res.data || res;                 // handle { data: {...} } or plain {...}
      ...
      setIsError(true);
      setMessage(
        error?.response?.data?.message || "Failed to load profile. Please refresh."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleAvatarChange = (e) => {
    ...
    if (file) {
      ...
      // NOTE: kung gusto mong i-upload sa backend, kailangan ng hiwalay na endpoint (multipart/form-data)
    }
  };
    ...
    try {
      if (isChangingPassword) {
        ...
        if (getPasswordStrength(user.newPassword) < 3) {
          setIsError(true);
          setMessage(
            "Password is too weak! Use at least 8 chars, mix of upper/lowercase, numbers, and symbols."
          );
          return;
        }
    ...
    <div className="min-h-screen bg-gray-50 flex justify-center items-start py-12 mt-8">
      <div className="flex flex-col md:flex-row gap-8 w-full max-w-6xl px-4">
        ...
        <div className="bg-white shadow-xl rounded-2xl p-8 w-full md:w-2/3 transition-transform hover:scale-105">
          ...
          {message && (
            <div
              className={`mb-4 text-center text-sm ${isError ? "text-red-600" : "text-green-600"
                }`}
            >
              {message}
            </div>
                    ...
                    {user.newPassword && (
                      <>
                        <div className="mt-1 h-2 rounded-full bg-gray-200 overflow-hidden">
                          <div
                            className={`h-2 transition-all duration-300 ${getPasswordStrength(user.newPassword) === 1
                              ? "w-1/6 bg-gray-500"
